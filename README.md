...
| 상태 종류                      | 도구        | 기준                                                                                            | 예시                                                                                                                                                          |
| ------------------------------ | ----------- | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 컴포넌트 로컬 상태             | `useState`  | 하나의 컴포넌트(또는 직계 자식)에서만 쓰는 값. 기본 선택지                                      | 폼 입력값, 드롭다운·모달 열림 여부                                                                                                                            |
...
- 새 store는 `src/stores/`에 `create<State>(...)` 형태로 추가합니다. 여러 컴포넌트가 이미 prop drilling으로 값을 전달하고 있다면 Zustand로 옮길 신호로 봅니다.
- Context를 새로 만들 때는 값(자주 바뀜)과 setter(안 바뀜)를 별도 Context로 분리해, 값이 바뀔 때 setter만 구독하는 컴포넌트까지 리렌더되지 않게 합니다.
