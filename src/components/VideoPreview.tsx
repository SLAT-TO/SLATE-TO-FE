...
function PreviewBox({ children }: { children: ReactNode }) {
  return (
    <div className="bg-neutral-4 flex aspect-video w-full flex-col items-center justify-center gap-2 rounded-xl">
      {children}
    </div>
  )
}
...
export default function VideoPreview({ videoUrl }: VideoPreviewProps) {
  ...
  if (loading) {
    return (
      <PreviewBox>
        <div className="border-neutral-3 border-t-primary h-6 w-6 animate-spin rounded-full border-2" />
      </PreviewBox>
    )
  }
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      <p className="text-body-sm text-neutral-10 line-clamp-2 font-semibold">{data.title}</p>
      {data.author_name && <p className="text-caption-lg text-neutral-5">{data.author_name}</p>}
    </div>
  )
}
